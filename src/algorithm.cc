/* 
   ...
   $Id: algorithm.cc,v 1.174 2008/06/26 12:07:59 peekas Exp $
...
consistency_error::consistency_error(const std::string& str)
	: logic_error(str)
	{
	}

algorithm_interrupted::algorithm_interrupted()
	: logic_error("Ctrl-C pressed")
	{
	}

algorithm_interrupted::algorithm_interrupted(const std::string& str)
	: logic_error("Ctrl-C pressed while executing "+str)
	{
	}
...
void algorithm::apply(unsigned int lue, bool multiple, bool until_nochange, bool make_copy, int act_at_level, 
	...
	if(tr.number_of_children(this_command)>=1) {
		...
		if(chld->fl.bracket==str_node::b_round && *this_command->name!="@") { // exception for '@(1)'
			...
			if(actold==tr.end()) {
				throw consistency_error("Expression ("
...
//				chld->fl.bracket=str_node::b_none; // otherwise the square brackets stay forever
			...
			txtout << "applying algorithm on marks (if any)." << std::endl;
			}
		}
...
//					txtout << "cancelling modification done" << std::endl;
		...
		if(is_output_module)
			global_success=g_applied;
//		txtout << *this_command->name << " completed, " << (subtree!=tr.end()) << " " 
//					 << expression_modified << std::endl;
...
//					wit=tr.next_at_same_depth(wit);
			...
			if(interrupted) 
				throw algorithm_interrupted("apply_recursive");
...
bool algorithm::check_consistency(iterator it) const
	...
	while(it!=end) {
		if(interrupted)
			throw algorithm_interrupted("check_consistency");

		if(*it->name=="\\sum") {
			if(*it->multiplier!=1)
				throw consistency_error("Found \\sum node with non-unit multiplier.");
			else if(exptree::number_of_children(it)<2)
				throw consistency_error("Found a \\sum node with 0 or 1 child nodes.");
			else { 
			 ...
			 if(exptree::number_of_children(it)<=1) 
				  throw consistency_error("Found \\prod node with only 0 or 1 children.");
			...
			while(ch!=it.end()) {
				if(*ch->name!="\\prod" && *ch->name!="\\sum") {
					if(ch->fl.bracket!=firstbracket)
						throw consistency_error("Found \\prod node with different brackets on its children.");
					}
				if(*ch->multiplier!=1) {
					throw consistency_error("Found \\prod child with non-unit multiplier.");
...
//					break;
			...
			if(exptree::number_of_children(it)!=2)
				throw consistency_error("Found \\sequence node with incorrect (non-2) number of children.");
			}
		++it;
		}
...
//			txtout << "going to use " << str.str() << std::endl;
	...
	assert(dd);
	throw consistency_error("Ran out of dummy indices for type \""+dd->set_name+"\".");
	}
	...
	if(*it->name=="\\sum" || *it->name=="\\equals") {
                  ...
                  // integer, coordinate or symbol indices always ok
						if(fri->second->is_integer()==false && !cdn && !smb) { 
							if(term_free.count((*fri).first)==0) {
								...
								if(*it->name=="\\sum") 
									throw consistency_error("Free indices in different terms in a sum do not match.");
								else
									throw consistency_error("Free indices on lhs and rhs do not match.");
								}
							}
						++fri;
						}
		...
		while(sit!=it.end()) {
			if(sit->is_index()==false) {
				...
				determine_intersection(factor_free, ind_dummy, must_be_empty);
				if(must_be_empty.size()>0) 
					 throw consistency_error("Triple index " 
				...
				determine_intersection(factor_dummy, ind_dummy, must_be_empty);
				if(must_be_empty.size()>0)
					throw consistency_error("Double index pair " 
													+ *(must_be_empty.begin()->second->name) 
													+ " inside a single factor found.");
					...
					if(sit->is_integer() || cdn || smb) {
						...
						if(fnd!=item_free.end()) {
							if(item_dummy.find(exptree(sit))!=item_dummy.end())
								throw consistency_error("Triple index " + *sit->name + " inside a single factor found.");
