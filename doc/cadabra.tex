...
\vfill
\noindent {\large\bf Copyright \copyright~2001--2010 ~Kasper Peeters}\\[3ex]
...
As a general rule of thumb, index patterns include their parent
relations. That is, patterns on the right-hand side of a substitution
rule do not get replaced if their parent relation does not match the
...
a Q_{c d};
\end{screen}
However, the same logic does not hold true for explicit wildcards,
\begin{screen}{1,2,3}
q_{c d};
@substitute!(%)( q_{a? b?} -> a? Q_{a? b?} );
c Q_{c d};
...
\subsalg{prodrule} acts on objects which carry a \subsprop{Derivative}
property, but it also acts on \subsalg{cdb\_Derivative} objects. This
allows e.g.~the \subsalg{vary} algorithm to wrap a derivative-like
object around powers and have \subsalg{prodrule} expand that
